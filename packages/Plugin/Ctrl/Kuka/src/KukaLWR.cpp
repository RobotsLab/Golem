...
void KukaLWRChain::ChainModel::create() {
	// TODO at some point
}

// Kuka custom froward kinematics
void KukaLWRChain::ChainModel::chainForwardTransform(const Mat34& localPose, const Real* cc, Mat34& trn) const {
	Mat34 tmp;
...
void KukaLWRChain::ChainModel::jointForwardTransform(const Mat34& localPose, const Real* cc, Mat34* trn) const {
	ASSERT(trn != NULL)
	Mat34 tmp;
...
void KukaLWRChain::ChainModel::jacobianSpatial(const Real* cc, Twist* jac) const {
	Mat34 tmp, trn;
	trn.setId();
...
KukaLWRChain::KukaLWRChain(Controller& controller) : Chain(controller) {
}

void KukaLWRChain::create(const Desc& desc) {
	Chain::create(desc); // throws
	setChainModel(desc.chainModel);
}

void KukaLWRChain::setChainModel(const ChainModel& chainModel) {
	if (!chainModel.isValid())
		throw MsgChainInvalidDesc(Message::LEVEL_CRIT, "KukaLWRChain::setChainModel(): Invalid chain model");
...
void KukaLWRChain::chainForwardTransform(const Real* cc, Mat34& trn) const {
	if (customKinematics)
...
void KukaLWRChain::jointForwardTransform(const Real* cc, Mat34* trn) const {
	if (customKinematics)
...
void KukaLWRChain::jacobianSpatial(const Real* cc, Twist* jac) const {
	if (customKinematics)
...
const Controller::ReservedOffset KukaLWR::RESERVED_OFFSET[RESERVED_INDEX_SIZE] = {
	...
	ReservedOffset("Force/Torque", ReservedOffset::TYPE_CONFIGSPACE, sizeof(Real), KukaLWRChain::NUM_JOINTS),
	/** Joint stiffness (Configspace, Real) */
	ReservedOffset("Stiffness", ReservedOffset::TYPE_CONFIGSPACE, sizeof(Real), KukaLWRChain::NUM_JOINTS),
	/** Joint damping (Configspace, Real) */
	ReservedOffset("Damping", ReservedOffset::TYPE_CONFIGSPACE, sizeof(Real), KukaLWRChain::NUM_JOINTS),
	...
	ReservedOffset("JntImp Next Pos", ReservedOffset::TYPE_CONFIGSPACE, sizeof(Real), KukaLWRChain::NUM_JOINTS),
	/** In joint impedence mode - offset on the torque value*/
	ReservedOffset("JntImp Add Torque", ReservedOffset::TYPE_CONFIGSPACE, sizeof(Real), KukaLWRChain::NUM_JOINTS),
	/** In cartesian impedance mode - desired position*/
	ReservedOffset("CartImp Next Pos", ReservedOffset::TYPE_DEFAULT, sizeof(Real), KukaLWR::SE3_DIM),
    /** In cartesian impedance mode - desired stiffness*/
	ReservedOffset("CartImp Stiffness", ReservedOffset::TYPE_DEFAULT, sizeof(Real), KukaLWR::TWIST_DIM),
	/** In cartesian impedance mode - desired damping*/
	ReservedOffset("CartImp Damping", ReservedOffset::TYPE_DEFAULT, sizeof(Real), KukaLWR::TWIST_DIM),
    /** In cartesian impedance mode - offset on the end effector force and torque*/
	ReservedOffset("CartImp Add Tcp FT", ReservedOffset::TYPE_DEFAULT, sizeof(Real), KukaLWR::TWIST_DIM),
	/** In cartesian impedance mode - joint null space control*/
	ReservedOffset("CartImp Jnt Null Space", ReservedOffset::TYPE_CONFIGSPACE, sizeof(Real), KukaLWRChain::NUM_JOINTS),
...
KukaLWR::KukaLWR(golem::Context& context) : SingleCtrl(context) {
}

//------------------------------------------------------------------------------

void KukaLWR::setToDefault(State& state) const {
...
void KukaLWR::setStateInfo(const Controller::Desc& desc) {
...
ptrdiff_t KukaLWR::getReservedOffset(U32 type) const {
	return reservedOffset[type];
}

void KukaLWR::lookupState(SecTmReal t, State &state) const {
	SingleCtrl::lookupState(t, state);

    //Todo Reserved area
}

void KukaLWR::interpolateState(const State& prev, const State& next, SecTmReal t, State& state) const {
