...
#include <Golem/App/ExPlacement/ExPlacement.h>
...
const std::string AppExPlacement::ID_ANY = "Any";

const std::string AppExPlacement::Data::ModeName[MODE_LAST + 1] = {
	"Model",
	"Query",
	"Solution",
};

data::Data::Ptr AppExPlacement::Data::Desc::create(golem::Context &context) const {
	golem::data::Data::Ptr data(new AppExPlacement::Data(context));
	static_cast<AppExPlacement::Data*>(data.get())->create(*this);
	return data;
}

AppExPlacement::Data::Data(golem::Context &context) : golem::Player::Data(context), owner(nullptr) {
}

void AppExPlacement::Data::create(const Desc& desc) {
...
void AppExPlacement::Data::setOwner(Manager* owner) {
	golem::Player::Data::setOwner(owner);
	this->owner = is<AppExPlacement>(owner);
	if (!this->owner)
		throw Message(Message::LEVEL_CRIT, "golem::AppExPlacement::Data::setOwner(): unknown data owner");
...
AppExPlacement::Data::Training::Map::iterator AppExPlacement::Data::getTrainingItem() {
...
void AppExPlacement::Data::setTrainingItem(Training::Map::const_iterator ptr) {
	U32 indexType = 0;
...
void AppExPlacement::Data::createRender() {
	Player::Data::createRender();
	{
		...
		if (mode == MODE_MODEL) {
			...
			if (ptr != training.end()) {
				Training::Waypoint::Seq::const_iterator dataptr;
				for (dataptr = ptr->second.waypoints.begin(); dataptr != ptr->second.waypoints.end(); dataptr++) {
					golem::Contact3D::draw(owner->contactAppearance, dataptr->contacts.model, modelFrame, owner->modelRenderer);
					for (auto &i : dataptr->points)
		...
		// query data
			...
			if (!densities.empty()) {
				if (queryShowDensities) {
					for (Density::denMap::const_iterator denIt = densities.begin(); denIt != densities.end(); denIt++) {
						const Density::Seq::const_iterator ptr = denIt->second.begin() + indexDensity;
			...
			if (!solutions.empty()) {
				for (Solution::solMap::const_iterator solPtr = solutions.begin(); solPtr != solutions.end(); solPtr++) {
					Solution::Seq solvec = solPtr->second;
					const Solution::Seq::iterator ptr = solvec.begin() + indexSolution;
					...
					for (Density::denMap::const_iterator denPtr = densities.begin(); denPtr != densities.end(); denPtr++) {
						Density::Seq denvec = denPtr->second;
						if (ptr->queryIndex < denvec.size()) {
							// F = P * r
							Mat34 trn = ptr->path[0].frame.toMat34() * owner->manipulator->getReferenceFrame();
							for (auto &i : denvec[ptr->queryIndex].points) {
								Vec3 p;
								trn.multiply(p, i);
...
void AppExPlacement::Data::load(const std::string& prefix, const golem::XMLContext* xmlcontext, const data::Handler::Map& handlerMap) {
	...
	try {
		...
		if (dataName.length() > 0) {
			...
			/*
			training.clear();
			frs.read(training, training.end(), std::make_pair(std::string(), Training()));
			/*
			densities.clear();
...
void AppExPlacement::Data::save(const std::string& prefix, golem::XMLContext* xmlcontext) const {
...
void AppExPlacement::PoseDensity::load(const golem::XMLContext* xmlcontext) {
...
void AppExPlacement::Data::Cluster::setToDefault(const Map& map, Counter& counter, const Training::Map& training, bool ordered) {
	counter.clear();
...
void AppExPlacement::Data::Cluster::setOccupied(const Map& map, Counter& counter, const std::string& type, golem::U32 index, bool ordered) {
	// find cluster
	Data::Cluster::Map::const_iterator cluster = map.find(type);
	if (cluster == map.end())
		throw Message(Message::LEVEL_NOTICE, "AppExPlacement::Data::Cluster::setOccupied(): %s type does not belong to any cluster", type.c_str());
	// add index
	counter[cluster->second.slot].insert(index);
	if (ordered)
...
bool AppExPlacement::Data::Cluster::isOccupied(const Map& map, const Counter& counter, const std::string& type, golem::U32 index) {
	// find cluster
	Data::Cluster::Map::const_iterator cluster = map.find(type);
	if (cluster == map.end())
		throw Message(Message::LEVEL_NOTICE, "AppExPlacement::Data::Cluster::isOccupied(): %s type does not belong to any cluster", type.c_str());
	// find slot
...
void AppExPlacement::Optimisation::load(const golem::XMLContext* xmlcontext) {
...
void AppExPlacement::Desc::load(golem::Context& context, const golem::XMLContext* xmlcontext) {
...
golem::AppExPlacement::AppExPlacement(Scene &scene) :
	Player(scene),
...
golem::AppExPlacement::~AppExPlacement() {
}

//------------------------------------------------------------------------------

golem::Camera* AppExPlacement::getWristCamera(const bool dontThrow) const
{
...
golem::Mat34 AppExPlacement::getWristPose() const
{
...
golem::Controller::State::Seq AppExPlacement::getTrajectoryFromPose(const golem::Mat34& w, const SecTmReal duration)
{
...
golem::ConfigMat34 AppExPlacement::getConfigFromPose(const golem::Mat34& w)
{
...
golem::Controller::State AppExPlacement::lookupStateArmCommandHand() const
{
...
void AppExPlacement::setHandConfig(Controller::State::Seq& trajectory, const golem::ConfigMat34& handPose)
{
...
void AppExPlacement::gotoWristPose(const golem::Mat34& w, const SecTmReal duration)
{
...
void AppExPlacement::gotoPose2(const ConfigMat34& pose, const SecTmReal duration)
{
	context.debug("AppExPlacement::gotoPose2: %s\n", toXMLString(pose).c_str());
...
void AppExPlacement::releaseHand(const double openFraction, const SecTmReal duration)
{
...
void AppExPlacement::closeHand(const double closeFraction, const SecTmReal duration)
{
	// @@@ HACK @@@
	...
	//context.debug("AppExPlacement::closeHand: finalPose: %s\n", toXMLString(finalPose).c_str());
...
void AppExPlacement::liftWrist(const double verticalDistance, const SecTmReal duration)
{
...
void AppExPlacement::nudgeWrist()
{
...
void AppExPlacement::rotateObjectInHand()
{
	auto showPose = [&](const std::string& description, const golem::Mat34& m) {
		context.write("%s: p={(%f, %f, %f)}, R={(%f, %f, %f), (%f, %f, %f), (%f, %f, %f)}\n", description.c_str(), m.p.x, m.p.y, m.p.z, m.R.m11, m.R.m12, m.R.m13, m.R.m21, m.R.m22, m.R.m23, m.R.m31, m.R.m32, m.R.m33);
	};
...
void golem::AppExPlacement::create(const Desc& desc) {
	desc.assertValid(Assert::Context("golem::AppExPlacement::Desc."));
	...
	if (!modelCamera)
		throw Message(Message::LEVEL_CRIT, "golem::AppExPlacement::create(): unknown model pose estimation camera: %s", desc.modelCamera.c_str());
	...
	if (!modelHandler)
		throw Message(Message::LEVEL_CRIT, "golem::AppExPlacement::create(): unknown model data handler: %s", desc.modelHandler.c_str());
	...
	if (!modelHandlerTrj)
		throw Message(Message::LEVEL_CRIT, "golem::AppExPlacement::create(): unknown model trajectory handler: %s", desc.modelHandlerTrj.c_str());
	...
	if (!queryCamera)
		throw Message(Message::LEVEL_CRIT, "golem::AppExPlacement::create(): unknown query pose estimation camera: %s", desc.queryCamera.c_str());
	...
	if (!queryHandler)
		throw Message(Message::LEVEL_CRIT, "golem::AppExPlacement::create(): unknown query data handler: %s", desc.queryHandler.c_str());
	...
	if (!queryHandlerTrj)
		throw Message(Message::LEVEL_CRIT, "golem::AppExPlacement::create(): unknown query trajectory handler: %s", desc.queryHandlerTrj.c_str());
	...
	if (!graspSensorForce)
		throw Message(Message::LEVEL_CRIT, "golem::AppExPlacement::create(): unknown grasp F/T sensor: %s", desc.graspSensorForce.c_str());
	...
	if (!objectCamera)
		throw Message(Message::LEVEL_CRIT, "golem::AppExPlacement::create(): unknown object capture camera: %s", desc.objectCamera.c_str());
	...
	if (!objectHandlerScan)
		throw Message(Message::LEVEL_CRIT, "golem::AppExPlacement::create(): unknown object (scan) data handler: %s", desc.objectHandlerScan.c_str());
	...
	if (!objectHandler)
		throw Message(Message::LEVEL_CRIT, "golem::AppExPlacement::create(): unknown object (process) data handler: %s", desc.objectHandler.c_str());
	...
	menuCmdMap.insert(std::make_pair("(", [&]() {
		if (to<Data>(dataCurrentPtr)->mode == Data::MODE_MODEL)
			...
			for (Data::Density::denMap::const_iterator denPtr = to<Data>(dataCurrentPtr)->densities.begin(); denPtr != to<Data>(dataCurrentPtr)->densities.end(); denPtr++) {
				Data::Density::Seq denvec = denPtr->second;
				...
				context.write("Query type %s, Item #%u\n", denvec[to<Data>(dataCurrentPtr)->indexDensity].type.c_str(), to<Data>(dataCurrentPtr)->indexDensity + 1);
			}
		}
			...
			for (Data::Solution::solMap::const_iterator solPtr = to<Data>(dataCurrentPtr)->solutions.begin(); solPtr != to<Data>(dataCurrentPtr)->solutions.end(); solPtr++) {
				Data::Solution::Seq solvec = solPtr->second;
				to<Data>(dataCurrentPtr)->indexSolution = to<Data>(dataCurrentPtr)->indexSolution <= 0 ? solvec.size() - 1 : to<Data>(dataCurrentPtr)->indexSolution - 1;
				const Data::Solution& solution = solvec[to<Data>(dataCurrentPtr)->indexSolution];
	...
	menuCmdMap.insert(std::make_pair(")", [&]() {
		if (to<Data>(dataCurrentPtr)->mode == Data::MODE_MODEL)
			...
			for (Data::Solution::solMap::const_iterator solPtr = to<Data>(dataCurrentPtr)->solutions.begin(); solPtr != to<Data>(dataCurrentPtr)->solutions.end(); solPtr++) {
				Data::Solution::Seq solvec = solPtr->second;
				to<Data>(dataCurrentPtr)->indexSolution = to<Data>(dataCurrentPtr)->indexSolution < solvec.size() - 1 ? to<Data>(dataCurrentPtr)->indexSolution + 1 : 0;
				const Data::Solution& solution = solvec[to<Data>(dataCurrentPtr)->indexSolution];
				context.write("Solution type %s (%u/%u), Item #%u, Likelihood_{total, contact, pose, collision}={%.6e, %.6e, %.6e, %.6e}\n", solution.type.c_str(), solution.queryIndex + 1, to<Data>(dataCurrentPtr)->densities.size(), to<Data>(dataCurrentPtr)->indexSolution + 1, solution.likelihood.likelihood, solution.likelihood.contact, solution.likelihood.pose, solution.likelihood.collision);
			}
		}
	...
	menuCmdMap.insert(std::make_pair("RMA", [=]() {
		...
		for (bool finish = false; !finish;) {
			...
			case 'G': {
				...
				// go to first contact pose
				gotoConfig(*contactPtr->second.waypoints.begin()->state);
				// done here
				context.write("Done!\n");
				break;
			}
			case 'C': {
				...
				if (model->second->create(*features, to<Data>(dataCurrentPtr)->modelFrame, modelMesh, contacts)) {
					...
					Data::Training::Map::iterator ptr = to<Data>(dataCurrentPtr)->training.find(modelType);
					if (ptr == to<Data>(dataCurrentPtr)->training.end()) {
					...
					Data::Training::Waypoint newdata(to<Data>(dataCurrentPtr)->modelFrame, contacts, pointsTrn, WaypointCtrl::lookup(*controller).state);
					context.debug("Frame: %d, Contacts: %d, Points: %d, State: %d", newdata.frame, newdata.contacts, newdata.points, newdata.state);
					ptr = to<Data>(dataCurrentPtr)->training.find(modelType);
					ptr->second.insert(newdata);
					createRender();
				}
				// done here
				context.write("Done!\n");
	...
	menuCmdMap.insert(std::make_pair("RMR", [=]() {
		...
		// load model object item
		{
			RenderBlock renderBlock(*this);
			golem::CriticalSectionWrapper cswData(scene.getCS());
			AppExPlacement::Data::Training::Map::iterator ptr = to<Data>(dataCurrentPtr)->getTrainingItem();
	...
	menuCmdMap.insert(std::make_pair("RD", [=]() {
		...
		for (;;) {
			...
			breakPoint("computed features and add to data bundle");
			// wrist frame
			const Mat34 frame = forwardTransformArm(WaypointCtrl::lookup(*controller).state);
			breakPoint("created wrist frame");
			// create query densities
			createQuery(ptr->second, frame, &to<Data>(dataCurrentPtr)->clusterCounter);
			breakPoint("created query densities");
			// generate wrist pose solutions
			generateSolutions();
			breakPoint("generated solutions");
			// select best trajectory
			selectTrajectory();
			breakPoint("selected trajectory");
...
golem::data::Item::Map::iterator golem::AppExPlacement::estimatePose(Data::Mode mode) {
...
// return ptr to Item
golem::data::Item::Map::iterator golem::AppExPlacement::objectGraspAndCapture(const bool stopAtBreakPoint)
{
	const auto breakPoint = [=](const char* str) {
		if (stopAtBreakPoint) {
			if (option("YN", makeString("%s: Continue (Y/N)...", str).c_str()) != 'Y')
	...
	data::Item::Map::iterator ptr;
	data::Item::Map& itemMap = to<Data>(dataCurrentPtr)->itemMap;
	if (option("YN", makeString("Skip Object Capture and use pre-loaded data? (Y/N)...").c_str()) != 'Y') 
		ptr = itemMap.find(objectItemScan);
...
golem::data::Item::Map::iterator golem::AppExPlacement::objectProcess(golem::data::Item::Map::iterator ptr) {
	// generate features
...
std::string golem::AppExPlacement::getTrajectoryName(const std::string& prefix, const std::string& type) const {
	return prefix + dataDesc->sepName + type;
}

//------------------------------------------------------------------------------

void golem::AppExPlacement::createQuery(golem::data::Item::Ptr item, const golem::Mat34& frame, const Data::Cluster::Counter* clusterCounter) {
	// Features
	const data::Feature3D* features = is<data::Feature3D>(item.get());
	if (!features)
		throw Message(Message::LEVEL_ERROR, "AppExPlacement::createQuery(): No query features");
	// training data are required
	if (to<Data>(dataCurrentPtr)->training.empty())
		throw Message(Message::LEVEL_ERROR, "AppExPlacement::createQuery(): No model densities");
	// model object pose
	if (to<Data>(dataCurrentPtr)->modelVertices.empty() || to<Data>(dataCurrentPtr)->modelVertices.empty())
		throw Message(Message::LEVEL_ERROR, "AppExPlacement::createQuery(): No model pose");
	// query object pose
	if (to<Data>(dataCurrentPtr)->queryVertices.empty() || to<Data>(dataCurrentPtr)->queryVertices.empty())
		throw Message(Message::LEVEL_ERROR, "AppExPlacement::createQuery(): No query pose");

	// select query any
	const golem::Query::Map::const_iterator queryAny = queryMap.find(ID_ANY);
	if (queryAny == queryMap.end())
		throw Message(Message::LEVEL_ERROR, "AppExPlacement::createQuery(): Unable to find Query %s", ID_ANY.c_str());
	// select pose std dev any
	const PoseDensity::Map::const_iterator poseAny = poseMap.find(ID_ANY);
	if (poseAny == poseMap.end())
		throw Message(Message::LEVEL_ERROR, "AppExPlacement::createQuery(): Unable to find pose density %s", ID_ANY.c_str());
	...
	for (Data::Training::Map::const_iterator i = to<Data>(dataCurrentPtr)->training.begin(); i != to<Data>(dataCurrentPtr)->training.end(); ++i) {
		Data::Density::Seq denVector;
		for (Data::Training::Waypoint::Seq::const_iterator dataIt = i->second.waypoints.begin(); dataIt != i->second.waypoints.end(); ++dataIt) {
			...
			try {
				...
				if (clusterCounter && Data::Cluster::isOccupied(clusterMap, *clusterCounter, i->first, index)) {
					...
					denVector.push_back(density);
					context.debug("AppExPlacement::createQuery(): slot %s index %u is occupied\n", i->first.c_str(), index);
					continue;
				}
				else
					context.debug("AppExPlacement::createQuery(): slot %s index %u is empty\n", i->first.c_str(), index);
			}
			catch (const Message& msg) {
				context.write(msg);
			}
			...
			try {
				...
				//	throw Message(Message::LEVEL_ERROR, "AppExPlacement::createQuery(): Unable to normalise model distribution");
				// clusters
				...
				object.reset(new Point3DKernel::Seq(clusterPoints.empty() ? features->getSize() : clusterPoints.size()));
				...
				query->second->setObject(object);
				query->second->setNNSearch(nnSearch);
				query->second->create(dataIt->contacts, *features);
			}
			...
			golem::data::Item::Map::iterator ptr = to<Data>(dataCurrentPtr)->itemMap.find(trjName);
			if (ptr == to<Data>(dataCurrentPtr)->itemMap.end())
				throw Message(Message::LEVEL_ERROR, "AppExPlacement::createQuery(): Empty trajectory");
			data::Trajectory* trajectory = is<data::Trajectory>(ptr);
			if (!trajectory)
				throw Message(Message::LEVEL_ERROR, "AppExPlacement::createQuery(): Trajectory handler does not implement data::Trajectory");
			// waypoints
			const WaypointCtrl::Seq waypoints = trajectory->getWaypoints();
			if (waypoints.size() < 2)
				throw Message(Message::LEVEL_ERROR, "AppExPlacement::createQuery(): Trajectory must have at least two waypoints");

			// Desired trajectory frame at contact pose
			const golem::Mat34 trajectoryFrame = forwardTransformArm(*dataIt->state);
			...
			if (frameDist.lin < REAL_EPS/* || frameDist.ang < REAL_EPS*/)
				throw Message(Message::LEVEL_ERROR, "AppExPlacement::createQuery(): Invalid distance between waypoints");
			...
			if (!golem::Sample<Real>::normalise<golem::Ref1>(density.pose))
				throw Message(Message::LEVEL_ERROR, "AppExPlacement::createQuery(): Unable to normalise pose distribution");
		...
		to<Data>(dataCurrentPtr)->densities.insert(std::make_pair(i->first, denVector));
	}

	for (Data::Density::denMap::iterator denPtr = to<Data>(dataCurrentPtr)->densities.begin(); denPtr != to<Data>(dataCurrentPtr)->densities.end(); denPtr++) {
		Data::Density::Seq& denVec = denPtr->second;
		if (denVec.empty())
			throw Message(Message::LEVEL_ERROR, "AppExPlacement::createQuery(): No query created");

		if (!golem::Sample<Real>::normalise<golem::Ref1>(denVec))
			throw Message(Message::LEVEL_ERROR, "AppExPlacement::createQuery(): Unable to normalise query distributions");
		context.debug("denPtr: %d, denvec: %d, denvec size: %d\n", denPtr, denVec, denVec.size());
	}
}

void golem::AppExPlacement::generateSolutions() {
	// training data are required
	if (to<Data>(dataCurrentPtr)->densities.empty())
		throw Message(Message::LEVEL_ERROR, "AppExPlacement::generateSolutions(): No query densities");
	for (Data::Density::denMap::const_iterator denPtr = to<Data>(dataCurrentPtr)->densities.begin(); denPtr != to<Data>(dataCurrentPtr)->densities.end(); ++denPtr) {
		Data::Density::Seq denvec = denPtr->second;
		Data::Solution::Seq solvec;
		...
		solvec.resize(optimisation.runs);
		Data::Solution::Seq::iterator ptr = solvec.begin();
		CriticalSection cs;
		ParallelsTask(context.getParallels(), [&](ParallelsTask*) {
			...
			for (;;) {
				...
				for (;;) {
					//context.debug("sampling QD\n");
					query = golem::Sample<golem::Real>::sample<golem::Ref1, Data::Density::Seq::const_iterator>(denvec, rand);
					if (query == denvec.end()) {
						context.error("AppExPlacement::generateSolutions(): Query density sampling error, denvec size: %d\n", denvec.size());
						return;
					}
					context.debug("sampled QD\n");
					break;
				}
				// sample pose density
				golem::Query::Pose::Seq::const_iterator pose;
				for (;;) {
					//context.debug("sampling PD\n");
					pose = golem::Sample<golem::Real>::sample<golem::Ref1, golem::Query::Pose::Seq::const_iterator>(query->pose, rand);
					if (pose == query->pose.end()) {
						context.error("AppExPlacement::generateSolutions(): Pose density sampling error\n");
						continue;
					}
					context.debug("sampled PD\n");
					break;
				}
				// set
				//context.debug("set\n");
				test.type = query->type;
				test.queryIndex = (U32)(query - denvec.begin());

				// local search: try to find better solution using simulated annealing
				context.debug("Performing SA\n");
		...
		context.debug("Pre-sort\n");
		// sort
		sortSolutions(solvec);

		// print debug information
		context.debug("AppExPlacement::generateSolutions(): time=%.6f, solutions=%u, steps=%u, energy=%f, greedy_accept=%d, SA_accept=%d\n", context.getTimer().elapsed() - t, optimisation.runs, optimisation.steps, optimisation.saEnergy, acceptGreedy, acceptSA);
		context.debug("Inserting %d\n", solvec);
		to<Data>(dataCurrentPtr)->solutions.insert(std::make_pair(denPtr->first, solvec));
	}
}

void golem::AppExPlacement::sortSolutions(Data::Solution::Seq& seq) const {
	if (seq.empty())
		throw Message(Message::LEVEL_ERROR, "AppExPlacement::sortSolutions(): No solutions");
...
void golem::AppExPlacement::selectTrajectory() {
	if (to<Data>(dataCurrentPtr)->solutions.empty())
		throw Message(Message::LEVEL_ERROR, "AppExPlacement::selectTrajectory(): No solutions");
	for (Data::Solution::solMap::const_iterator solPtr = to<Data>(dataCurrentPtr)->solutions.begin(); solPtr != to<Data>(dataCurrentPtr)->solutions.end(); solPtr++){
		Data::Solution::Seq solvec = solPtr->second;
		const U32 testTrajectories = (U32)solvec.size();
		...
		const std::pair<U32, RBDistEx> val = manipulator->find<U32>(0, testTrajectories, [&](U32 index) -> RBDistEx {
			...
			createRender();
			Manipulator::Waypoint::Seq& path = solvec[index].path;
		...
		const Data::Solution& solution = solvec[val.first];
		createRender();
		...
		// add trajectory waypoint
		{
			...
			data::Trajectory* trajectory = is<data::Trajectory>(ptr);
			if (!trajectory)
				throw Message(Message::LEVEL_ERROR, "AppExPlacement::selectTrajectory(): Trajectory handler does not implement data::Trajectory");
			// add current state
			trajectory->setWaypoints(seq);
			Data::View::setItem(to<Data>(dataCurrentPtr)->itemMap, ptr, to<Data>(dataCurrentPtr)->getView());
		}
		...
		try {
			...
			const U32 index = (U32)std::distance(range.first, i);
			// set slot occupied
			context.debug("AppExPlacement::selectTrajectory(): slot %s index %u set occupied\n", solution.type.c_str(), index);
...
void golem::AppExPlacement::performTrajectory(bool testTrajectory) {
	golem::data::Item::Map::iterator ptr = to<Data>(dataCurrentPtr)->itemMap.find(queryItemTrj);
	if (ptr == to<Data>(dataCurrentPtr)->itemMap.end())
		throw Message(Message::LEVEL_ERROR, "AppExPlacement::performTrajectory(): Unable to find query trajectory");
	data::Trajectory* trajectory = is<data::Trajectory>(ptr);
	if (!trajectory)
		throw Message(Message::LEVEL_ERROR, "AppExPlacement::performTrajectory(): Trajectory handler does not implement data::Trajectory");

	Controller::State::Seq seqInv = WaypointCtrl::make(trajectory->getWaypoints(), true);
	if (seqInv.size() < 2)
		throw Message(Message::LEVEL_ERROR, "AppExPlacement::performTrajectory(): At least two waypoints required");
	// reverse
	...
	struct ProfileCallback : Profile::CallbackDist {
		const AppExPlacement* demo;
		ProfileCallback(const AppExPlacement* demo) : demo(demo) {}
	...
	if (profile == nullptr)
		throw Message(Message::LEVEL_ERROR, "AppExPlacement::performTrajectory(): unable to create profile");
	...
	if (handlerPtr == handlerMap.end())
		throw Message(Message::LEVEL_ERROR, "AppExPlacement::performTrajectory(): unknown default trajectory handler %s", getPlanner().trajectoryHandler.c_str());
	data::Handler* handler = is<data::Handler>(handlerPtr);
	if (!handler)
		throw Message(Message::LEVEL_ERROR, "AppExPlacement::performTrajectory(): invalid default trajectory handler %s", getPlanner().trajectoryHandler.c_str());
	itemTrajectory = handler->create();
	data::Trajectory* trajectoryIf = is<data::Trajectory>(itemTrajectory.get());
	if (!trajectoryIf)
		throw Message(Message::LEVEL_ERROR, "AppExPlacement::performTrajectory(): unable to create trajectory using handler %s", getPlanner().trajectoryHandler.c_str());
...
void golem::AppExPlacement::render() const {
	Player::render();
...
void golem::XMLData(golem::AppExPlacement::Data::Cluster::Map::value_type& val, golem::XMLContext* xmlcontext, bool create) {
	golem::XMLData("type", const_cast<std::string&>(val.first), xmlcontext, create);
	golem::XMLData("slot", val.second.slot, xmlcontext, create);
}

void golem::XMLData(golem::AppExPlacement::PoseDensity::Map::value_type& val, golem::XMLContext* xmlcontext, bool create) {
...
template <> void golem::Stream::read(golem::AppExPlacement::Data::Training::Map::value_type& value) const {
	read(const_cast<std::string&>(value.first));
	value.second.waypoints.clear();
	read(value.second.waypoints, value.second.waypoints.begin());
}

template <> void golem::Stream::write(const golem::AppExPlacement::Data::Training::Map::value_type& value) {
	write(value.first);
	write(value.second.waypoints.begin(), value.second.waypoints.end());
}

template <> void golem::Stream::read(golem::AppExPlacement::Data::Training::Waypoint& value) const {
	read(value.state);
	read(value.frame);
	value.contacts.model.clear();
	read(value.contacts.model, value.contacts.model.begin());
	value.points.clear();
	read(value.points, value.points.begin());
}

template <> void golem::Stream::write(const golem::AppExPlacement::Data::Training::Waypoint& value) {
	write(value.state);
	write(value.frame);
	write(value.contacts.model.begin(), value.contacts.model.end());
...
template <> void golem::Stream::read(golem::AppExPlacement::Data::Density::Seq::value_type& value) const {
	read(value.type);
	value.object.clear();
...
template <> void golem::Stream::write(const golem::AppExPlacement::Data::Density::Seq::value_type& value) {
	write(value.type);
...
template<> void golem::Stream::read(golem::AppExPlacement::Data::Density::denMap::value_type& value) const {
	read(const_cast<std::string&>(value.first));
	read(value.second, value.second.begin());
}

template <> void golem::Stream::write(const golem::AppExPlacement::Data::Density::denMap::value_type& value) {
	write(value.first);
	write(value.second.begin(), value.second.end());
}

template <> void golem::Stream::read(golem::AppExPlacement::Data::Solution::Seq::value_type& value) const {
	read(value.type);
	read(value.pose);
	value.path.clear();
	read(value.path, value.path.begin());
...
template <> void golem::Stream::write(const golem::AppExPlacement::Data::Solution::Seq::value_type& value) {
	write(value.type);
	write(value.pose);
	write(value.path.begin(), value.path.end());
...
template<> void golem::Stream::read(golem::AppExPlacement::Data::Solution::solMap::value_type& value) const {
	read(const_cast<std::string&>(value.first));
	read(value.second, value.second.begin());
}

template <> void golem::Stream::write(const golem::AppExPlacement::Data::Solution::solMap::value_type& value) {
	write(value.first);
	write(value.second.begin(), value.second.end());
}
...
int main(int argc, char *argv[]) {
	return golem::AppExPlacement::Desc().main(argc, argv);
}
